...
private def readToml (p : FilePath) : IO TomlMini.Table := do
  let txt ← IO.FS.readFile p
  match TomlMini.parse txt with
  | .ok t   => pure t
  | .error e => die s!"{p}: {e}"

private def readVerifierToml (p : FilePath) : IO VerifierToml := do
  let t ← readToml p
  match TomlMini.require t "lean_project" p.toString,
        TomlMini.require t "verification_folder" p.toString with
...
private def writeOriginToml (p : FilePath) (o : OriginToml) : IO Unit :=
  writeFile p <| TomlMini.render [("rust_project", o.rustProject)]
...
private def emitProgramFile
  ...
  let bodiesBlock := EmitMini.funcBodies m
  let moduleExpr := EmitMini.module m
  ...
  let driftBlock := EmitMini.driftCheck watRelPath watText.hash
  let lines := [
    "/-",
...
private def cmdCheck (path : String) : IO Unit := do
  ...
  | .ok m =>
    ...
    -- 6. lake build on the lean project
